                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('voorraad.producten.update', $product->id) }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="product_naam" value="{{ old('product_naam', $product->product_naam ?? '') }}"
                        ...
                        <div>
                            ...
                            <select name="categorie_id"
                                ...
                                @foreach($categorieen as $cat)
                                    @php
                                        $currentId = old('categorie_id', $product->categorie_id ?? null);
                                    @endphp
                                    <option value="{{ $cat->id }}" {{ (string) $currentId === (string) $cat->id ? 'selected' : '' }}>
                                        {{ $cat->naam }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" min="0" name="aantal_voorraad" value="{{ old('aantal_voorraad', $product->aantal_voorraad ?? 0) }}"
                                   class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">
